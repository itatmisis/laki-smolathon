...
async def route_create_location_list(location_list: list[NewLocation], current_user: UserOut = Depends(get_current_user)):
    await create_location_list(location_list)


@map_router.post('/category')
async def route_create_category_list(category_list: list[NewCategory], current_user: UserOut = Depends(get_current_user)):
